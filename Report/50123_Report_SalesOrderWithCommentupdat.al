report 50123 "Sales Order With Comment updat"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Report/50123_Report_SalesOrderWithCommentupdat.rdlc';
    Caption = 'Sales Order';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(AmountExclInvDisc1; ROUND(TotalAmount1, 0.01))
            {
            }
            column(TempSalesLineLineDiscountAmount1; ROUND(DiscountAmount, 0.01))

            {
            }
            column(TempSalesLineLineAmtTaxAmtInvDiscAmt1; ROUND(TotalAmount1, 0.01) - ROUND(InvDisc, 0.01))
            {
            }
            column(HideSection; HideSection)
            {
            }
            column(txtShipADD_1; txtShipADD[1])
            {
            }
            column(txtShipADD_2; txtShipADD[2])
            {
            }
            column(txtShipADD_3; txtShipADD[3])
            {
            }
            column(txtShipADD_4; txtShipADD[4])
            {
            }
            column(SalesHeader_RequestedDeliveryDate; FORMAT("Sales Header"."Requested Delivery Date"))
            {
            }
            ...
            dataitem("Sales Line"; "Sales Line")
            {
                ...
                begin
                    ...
                    /*
                    txtAlis := '';
                    recItemCust.SETRANGE("Customer No.","Sales Line"."Sell-to Customer No.");
                    recItemCust.SETRANGE("Item No.","Sales Line"."No.");
                    IF recItemCust.FIND('-') THEN BEGIN
                       txtAlis := recItemCust."Customer Item No.";
                    END;
                     */
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    //column(QRCode_SalesHeader; "Sales Header"."QR Code")//SC-TIC-62 Begin
                    ...
                    column(recCustomer_Contact; "Sales Header"."Sell-to Contact")
                    {
                    }
                    column(SelltoContactPhoneNo; SellToContact."Phone No.")
                    {
                    }
                    column(SelltocustomerEmail; SellToContact."E-Mail")
                    {
                    }
                    ...
                    column(ShiptoContactPhoneNo; ShipToContact."Phone No.")
                    {
                    }
                    column(ShipptocustomerEmail; ShipToContact."E-Mail")
                    {
                    }
                    ...
                    dataitem(SalesLine; "Integer")
                    {
                        ...
                        column(TempSalesLineLineDiscountAmpont; TempSalesLine."Line Discount Amount")
                        {
                        }
                        ...
                        begin
                            ...
                            WITH TempSalesLine DO BEGIN
                                ...
                                IF Type = Type::Item THEN BEGIN
                                    IF TempSalesLine.Comments <> '' THEN BEGIN
                                        IF "Comment Status" <> "Comment Status"::Complete THEN
                                            RemarksText := '~'
                                        ELSE
                                            RemarksText := '';
                                    END ELSE
            ...
            begin
                ...
                // << QR
                //QRMng.RUN("Sales Header"); //VR
                // QRMng.QROrder("Sales Header");
                "Sales Header".CALCFIELDS("QR Code");
                // >> QR
                ...
                IF recSalesLine.FIND('-') THEN
                    REPEAT
                        ...
                        InvDisc := InvDisc + ROUND(recSalesLine."Inv. Discount Amount", 0.001);
                    UNTIL recSalesLine.NEXT = 0;
                ...
                /*
                  IF "Completely Shipped" THEN
                   ...
                   ELSE IF blnComplete THEN
                     SalesOrderCaptionLbl:='Sales Order'
                    ELSE
                     SalesOrderCaptionLbl:='Back Order';
                 */

                IF blSalesInvoice THEN
                    SalesOrderCaptionLbl := 'Sales Invoice'
                ELSE
                    SalesOrderCaptionLbl := 'Sales Order';
                ...
                txtShipADD[1] := FORMAT("Sales Header"."Ship-to Name");
                ...
                IF ShipToContact.GET("Sales Header"."Sell-to Contact No.") THEN;

                BarcodeText := '';
                g_barcodeItem := "Sales Header"."No.";
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(HideSection; HideSection)
                    {
                        Caption = 'Hide Section';
                        ApplicationArea = all;
                    }
                    field(blSalesInvoice; blSalesInvoice)
                    {
    ...
    var
        ...
        InvDisc: Decimal;
        HideSection: Boolean;
        txtShipADD: array[8] of Text[255];
        txtAlis: Text[50];
