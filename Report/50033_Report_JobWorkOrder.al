report 50033 "Job Work Order"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Report/50033_Report_JobWorkOrder.rdlc';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            begin
                ...
                IF recSalesLine.FIND('-') THEN BEGIN
                   REPEAT
                    //recSalesLine."Completely Shipped";
                    IF (recSalesLine."Quantity Shipped">0) AND (NOT recSalesLine."Completely Shipped") THEN
                       blnComplete:=FALSE
                     ELSE
                       blnComplete:=TRUE;
                   UNTIL (recSalesLine.NEXT=0) OR (NOT blnComplete);
                  END;*/
                IF "Completely Shipped" THEN
                    SalesOrderCaptionLbl := 'Job Work'
                ELSE                                              //SPD MS 060815
                    SalesOrderCaptionLbl := 'Job Work';
                ...
                CLEAR(LocCountry);
                Rec5740.RESET;
                Rec5740.SETRANGE(Rec5740."Sales Order No", "Sales Header"."No.");
                IF Rec5740.FIND('-') THEN BEGIN
                    TrnsfToCode := Rec5740."Transfer-to Code";
                    Rec14.RESET;
                    IF Rec14.GET(TrnsfToCode) THEN BEGIN
                        RecName := Rec14.Name;
                        RecAdd := Rec14.Address;
                        LocCity := Rec14.City;
                        LocState := Rec14.County;
                        LocStCode := Rec14."Post Code";
                        LocCountry := Rec14."Country/Region Code";
                    END;
            ...
            begin
                Cinfo.GET;
                Cinfo.CALCFIELDS(Cinfo.Picture);   //SPD MS
            end;
        }
    }
