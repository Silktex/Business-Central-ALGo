report 50052 "Import Sales Order"
{
    ...
    begin
        ...
        IF GetValueAtCell(RowNo, 1) <> CustNo THEN BEGIN
            recSalesHeader.INIT;
            ...
            recSalesHeader.VALIDATE(recSalesHeader."Sell-to Customer No.", GetValueAtCell(RowNo, 1));
            ...
            recSalesHeader.VALIDATE(recSalesHeader."Location Code", GetValueAtCell(RowNo, 6));
        ...
        recSalesLine.SETRANGE(recSalesLine."Document No.", OrderNo);
        recSalesLine.SETRANGE(recSalesLine.Type, recSalesLine.Type::Item);
        recSalesLine.SETRANGE(recSalesLine."No.", GetValueAtCell(RowNo, 3));
        IF recSalesLine.FINDFIRST THEN BEGIN
            EVALUATE(decItemQty, GetValueAtCell(RowNo, 4));
            recSalesLine.VALIDATE(recSalesLine.Quantity, recSalesLine.Quantity + decItemQty);
            recSalesLine.MODIFY;
            IF GetValueAtCell(RowNo, 5) <> '' THEN BEGIN
                CreateReservEntry(recSalesLine, GetValueAtCell(RowNo, 5), decItemQty);
            END;

        END ELSE BEGIN
            recSalesLine1.RESET;
            recSalesLine1.SETRANGE(recSalesLine1."Document Type", recSalesLine1."Document Type"::Order);
            recSalesLine1.SETRANGE(recSalesLine1."Document No.", OrderNo);
            ...
            recSalesLine."Document No." := OrderNo;
            recSalesLine."Line No." := LineNo + 10000;
            EVALUATE(recSalesLine.Type, GetValueAtCell(RowNo, 2));
            recSalesLine.VALIDATE(recSalesLine."No.", GetValueAtCell(RowNo, 3));
            EVALUATE(decUnitPrice, GetValueAtCell(RowNo, 8));
            recSalesLine.VALIDATE(recSalesLine."Unit Price", decUnitPrice);
            EVALUATE(decItemQty, GetValueAtCell(RowNo, 4));
            recSalesLine.VALIDATE(recSalesLine.Quantity, decItemQty);
            EVALUATE(DecountAmount, GetValueAtCell(RowNo, 7));
