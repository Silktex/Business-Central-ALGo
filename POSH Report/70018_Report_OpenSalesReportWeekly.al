report 70018 "Open Sales Report Weekly"
{
    ProcessingOnly = true;
        ...
        IF SalesPerson.FINDFIRST THEN BEGIN
            REPEAT
                ...
                IF SalesHeader.FINDSET THEN BEGIN
                    ...
                    Name := 'Weekly_Open_Sales_Order_Report-' + NewString + '-' + CONVERTSTR(FORMAT(WORKDATE), '/', '-') + '.xlsx';
                    TempBlob.CreateOutStream(Outstream, TextEncoding::UTF8);
                    RecRef.GetTable(SalesHeader);
                    Report.SaveAs(70012, '', ReportFormat::Excel, Outstream, RecRef);
                    SLEEP(20000);

                    TempBlob.CreateInStream(InStream, TextEncoding::UTF8);
                    IF TempBlob.HasValue() THEN BEGIN
                        txtSubject := 'Weekly Open Sales Order Report ' + FORMAT(WORKDATE);
                        txtBody1 := '<html><body>'
                                    + '<pre><p><font face = "Arial" size = "4">Dear ' + SalesPerson.Name + ',</font></p></pre>'
                                    + '<pre><p><font face = "Arial" size = "4">Please find Weekly Open Sales Order Report for Last Week, </a></font></p></pre>'
